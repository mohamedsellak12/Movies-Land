...
const SeasonSwiper: React.FC<SeasonSwiperProps> = ({ items }) => {
  if (!items || items.length === 0) return null;
  const RealSeasons :any[]=items.filter((i)=>i.season_number!=0)

  return (
    <div className="mt-6 max-w-6xl mx-auto">
      <h2 className="text-xl sm:text-2xl font-semibold mb-4">Seasons ({RealSeasons.length})</h2>
      <Swiper
        spaceBetween={16}
        slidesPerView={2}
        navigation={true}
